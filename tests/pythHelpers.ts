...
async function createPriceFeed(
	...
	const ixs = [
		...
		}).signers([priceFeed]).instruction()
	]

	const sig = await anchor.getProvider().connection.sendTransaction(new Transaction().add(...ixs), [payer, priceFeed]);
...
export const setFeedPrice = async (
	...
	await oracleProgram.rpc.setPrice(new anchor.BN(newPrice * 10 ** -data.exponent), {
